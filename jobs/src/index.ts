  ...
  try {
    ...
    await sql`
      ...
      BEGIN
        ...
        END IF;
        IF NOT EXISTS (
          SELECT 1 FROM pg_type WHERE typname = 'wrok_location'
        ) THEN
          CREATE TYPE wrok_location AS ENUM ('On-site', 'Remote','Hybrid');
        END IF;
         IF NOT EXISTS (
    ...
    await sql`
      CREATE TABLE IF NOT EXISTS jobs (
        ...
        wrok_location wrok_location NOT NULL,
...
initDb().then(() => {
  app.listen(process.env.PORT, () => {
    console.log(`Job service is running on http://localhost:5003`)
  })
})
